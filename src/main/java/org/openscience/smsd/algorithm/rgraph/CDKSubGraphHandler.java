...
import java.util.*;
////@TestMethodimport org.openscience.cdk.annotations.TestClass;
////@TestMethodimport org.openscience.cdk.annotations.TestMethod;
 ...
 */
////@TestMethod@TestClass("org.openscience.cdk.smsd.algorithm.cdk.CDKMCSHandlerTest")
public class CDKSubGraphHandler implements IResults {

////@TestMethod    ////@TestMethod~--- fields -------------------------------------------------------------
    ...
    private boolean isSubgraph() {
        ...
        try {
            ...
            rmap = null;
////@TestMethod            System.err.println("WARNING: graphContainer: most probably time out error ");
        }
    ...
    protected IAtomContainerSet getUncommon(IAtomContainer mol, IAtomContainer mcss) throws CDKException {
        ...
        ////@TestMethod at this point we have the serial numbers of the bonds to delete
        ////@TestMethod we should get the actual bonds rather than delete by serial numbers
        ArrayList<IAtom> atomsToDelete = new ArrayList<>();
        for (Integer serial : atomSerialsToDelete) {
            atomsToDelete.add(mol.getAtom(serial));
        }

        ////@TestMethod now lets get rid of the bonds themselves
        for (IAtom atom : atomsToDelete) {
            mol.removeAtomAndConnectedElectronContainers(atom);
        }

        ////@TestMethod now we probably have a set of disconnected components
        ////@TestMethod so lets get a set of individual atom containers for
        ////@TestMethod corresponding to each component
        return ConnectivityChecker.partitionIntoMolecules(mol);
    }

    ////@TestMethod~--- get methods --------------------------------------------------------
    private synchronized void setAllMapping(List<Map<Integer, Integer>> solutions) {

        ////@TestMethodint count_final_sol = 1;
        ////@TestMethodSystem.out.println("Output of the final FinalMappings: ");
        try {
            int counter = 0;
